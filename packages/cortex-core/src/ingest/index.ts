...
export function createIngest(deps: IngestDeps): IngestApi {
  return {
    storeMemory: async (input: StoreMemoryInput): Promise<string> => {
      ...
      deps.logger.debug('cortex.ingest.storeMemory', { id: memory.id });
      return memory.id;
    },

    ingestDocument: async () => {
      throw new NotImplementedError(
        'cortex-core/ingest.ingestDocument is still a stub; lands when document ingestion is demanded by a consumer',
      );
    },
    ...
    extractFacts: async (memoryId: string): Promise<Fact[]> => {
      ...
      if (!memory) {
        deps.logger.debug('cortex.ingest.extractFacts.unknown-memory', { memoryId });
        return [];
      }
      ...
      for (const raw of rawFacts.slice(0, 3)) {
        ...
        try {
          ...
          deps.logger.debug('cortex.ingest.extractFacts.invalid-fact', {
            memoryId,
            error: (err as Error).message,
          });
        }
      ...
      deps.logger.debug('cortex.ingest.extractFacts', {
        memoryId,
